...
import { HiOutlineSearch, HiOutlineAdjustments, HiOutlineRefresh, HiOutlineExclamationCircle } from 'react-icons/hi';
...
const BrowseWorkers = () => {
  ...
  const fetchWorkers = async () => {
    ...
    try {
      ...
      setErrorNotice(
        e.isHtmlFallback
          ? 'Backend API not connected yet (VITE_API_URL). Showing demo workers.'
          : e.message || 'Connecting to backend timed out. Showing demo workers.'
      );

      // Filter demo workers locally so user can still test search/filter
      ...
      if (minRating > 0) {
        filtered = filtered.filter((w) => (w.rating || 0) >= minRating);
      }
      if (q) {
    ...
    <div className="max-w-7xl mx-auto px-5 md:px-8 py-14">
      <div className="mb-10">
        ...
        <p className="text-ink/60 mt-2 max-w-lg">
          Filter by trade, rating and search to find the right person for the job — verified, rated, ready.
        </p>
      </div>
      ...
      {isFallback && errorNotice && (
        <div className="mb-8 p-4 rounded-xl bg-hazard/10 border border-hazard/30 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2.5 text-sm text-ink/80">
            <HiOutlineExclamationCircle className="text-xl text-hazard shrink-0" />
            <span>
              <strong>Demo Mode:</strong> {errorNotice}
            </span>
          </div>
          <button
          ...
          >
            <HiOutlineRefresh /> Retry Live Connection
          </button>
        </div>
      )}
      ...
      <form onSubmit={onSearchSubmit} className="ticket p-5 shadow-card flex flex-wrap gap-4 items-center mb-10">
        ...
        <select
          ...
          className="border border-ink/15 rounded-lg px-3 py-2.5 outline-none focus:border-hazard bg-transparent"
        >
          {trades.map((t) => (
      ...
      {loading ? (
        ...
        <motion.div layout className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {workers.map((w, i) => (
            <WorkerCard key={w._id || i} worker={w} index={i} />
          ))}
        </motion.div>
      )}
    </div>
